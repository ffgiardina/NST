...
def compute_content_cost(content_output, generated_output):
    ...
    J_content = 1.0/(4*n_H*n_W*n_C)*tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled, a_G_unrolled)))
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = 1.0/(4*(n_H*n_W)**2*n_C**2)*tf.reduce_sum(tf.square(tf.subtract(GS, GG)))
...
@tf.function()  # Use decorator to make total_cost a tensorflow Function
...
def make_square(image):
    ...
    N = min((width, height))
    crop_range = ((width-N)//2, (height-N)//2, (width-N)//2+N, (height-N)//2+N)
    square_image = image.crop(crop_range)
    return square_image
